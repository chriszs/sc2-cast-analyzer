...
def read_box(frame,w,h,y,x,ocr,name):
    ...
    # ret,box = cv2.threshold(box,125,255,cv2.THRESH_BINARY_INV)
...
#start the video
cap = cv2.VideoCapture('sample2.mp4') # play.url)
while (True):
    ret,frame = cap.read()
    ...
    box1,text1 = read_box(frame,50,28,origH-72,x,i % 60 == 0,'box1')
    # box1,text1 = read_box(frame,w,28,origH-72,x,i % 60 == 0,'box1')

    # box2,text2 = read_box(frame,w,12,origH-43,x,i % 60 == 0,'box2')

    box3,text3 = read_box(frame,50,28,origH-30,x,i % 60 == 0,'box3')
